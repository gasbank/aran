...
int BwOpenGlWindow::handle( int eventType )
{
	...
	if (eventType == FL_PUSH)
	{
		...
		if (m_ac.sgPtr)
		{
			...
			ArnMakePickRay(&origin, &direction, float(mouseX), float(m_ac.avd.Height - mouseY), &modelview, &projection, &m_ac.avd);
			ArnMesh* mesh = reinterpret_cast<ArnMesh*>(m_ac.sgPtr->findFirstNodeOfType(NDT_RT_MESH));
			if (mesh)
			{
				bool bHit = false;
