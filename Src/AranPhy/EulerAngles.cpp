...
#ifdef _MSC_VER
#pragma warning(disable:4244 4305 4552)  // for VC++, no precision loss complaints
#endif
...
EulerAngles Eul_FromHMatrix(HMatrix M, int order)
{
	...
	if (sy > 16*FLT_EPSILON) {
		ea.x = dAtan2(M[i][j], M[i][k]);
	    ea.y = dAtan2(sy, M[i][i]);
	    ea.z = dAtan2(M[j][i], -M[k][i]);
	} else {
	    ea.x = dAtan2(-M[j][k], M[j][j]);
	    ea.y = dAtan2(sy, M[i][i]);
	    ea.z = 0;
	}
    } else {
	...
	if (cy > 16*FLT_EPSILON) {
	    ea.x = dAtan2(M[k][j], M[k][k]);
	    ea.y = dAtan2(-M[k][i], cy);
	    ea.z = dAtan2(M[j][i], M[i][i]);
	} else {
	    ea.x = dAtan2(-M[j][k], M[j][j]);
	    ea.y = dAtan2(-M[k][i], cy);
	    ea.z = 0;
