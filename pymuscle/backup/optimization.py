...
def DetermineContactPoints(cornersX, cornersXd):
	...
	for i in range(8): # 8 is the maximum number of contact points of a rigid box
		...
		if z < 0:
			# Serious happened
			print 'ERROR: PENETRATION OCCURRED !!! (z=%.16e)' % z
			#raise Exception, 'penetration'
		elif z < colliding_tolerance:
			# Calculate separation velocity of a point relative to the ground
			sep_vel = dot(array([0,0,1.]), cornersXd[0:3,i])
			if sep_vel < 0:
				# Impulse needed
				impulse_corner.append(i)
			elif sep_vel > 0:
				# Nothing is needed. Contact will broken.
				pass
			else:
	...
	return static_contacts, dynamic_contacts, impulse_corner
...
def AngularAccelerationComMatrix(cornersX, cornersXd, static_contacts, dynamic_contacts,
	...
	# World(inertial) frame inertia tensor
	H_com = dot(A, dot(H_com_bar, A.T))
	...
	if R is not 0 and VAI is not 0:
		A_alpha = dot( dot( H_com_inv, R ), VAI )
		S_lambda = BuildSelectionMatrix(n_cs, n_cd, n_u, 'lambda')
		A_alpha = dot(A_alpha, S_lambda)

	b_alpha = -dot(H_com_inv, cross(omega, dot(H_com, omega)))
	assert len(b_alpha) == 3
	return A_alpha, b_alpha, VAI
...
def LinearAccelerationMatrix(A_a_com, b_a_com, r_pq, A_alpha, b_alpha, omega):
	"""
	...
	구해진다. r_pq가 0인 경우 A_alpha, b_alpha와 omega의 값은 무시된다.
	"""
	A_a_q = A_a_com - dot(cross_op_mat(r_pq), A_alpha)
	b_a_q = b_a_com.flatten() + dot( dot(cross_op_mat(omega), cross_op_mat(omega)) + cross_op_mat(b_alpha), r_pq).flatten()
...
>>>>>>> .r532
	...
	for i in range(1, n_x):
		task.appendcone(mosek.conetype.quad, 0.0, [ 0, i ])
	"""
