...
from box_lcp3_exp import FrameMove, ExpBodyFromTestSet, GetTestSetCount
...
class TestContext( BaseContext ):
	...
	def OnInit( self ):
		...
		for i in range(GetTestSetCount()):
			self.addEventHandler( "keypress", name=str(i+1), function = self.OnPressTestSet)
		
		self.body = ExpBodyFromTestSet(0)
		self.nextTestSet = None
	...
	def OnTimerFraction( self, event ):
		...
		self.geometry.children[3].translation = self.body.q[0:3]
		rot = self.body.q[3:6]
		rotAngle = linalg.norm(rot)
		self.geometry.children[3].rotation    = (rot[0],rot[1],rot[2],rotAngle)

		if self.nextTestSet is not None:
			self.body = ExpBodyFromTestSet(self.nextTestSet)
			self.nextTestSet = None
			self.geometry.children[3].children[0].geometry = Box( size=self.body.boxsize )
		FrameMove(self.body)
	...
	def OnToggleTimer( self, event ):
		...
		if self.time.active:
			...
			self.time.resume()
	def OnPressTestSet( self, event ):
		"""Allow the user to pause/restart the timer."""
		self.nextTestSet = int(event.name)-1
