...
class PmBody:
    def __init__(self, name, pName, mass, boxsize, q, qd, dc, rotParam):
        ...
        q   : linear & angular position (6 or 7 dimension)
        qd  : linear & angular velocity (6 or 7 dimension) -- which parameterization?
        dc  : drawing color
        """
        sx, sy, sz = boxsize
        ...
        self.corners = [ ( sx/2,  sy/2,  sz/2),
                         ...
                         (-sx/2, -sy/2,  sz/2),
                         (-sx/2, -sy/2, -sz/2) ]
        assert(rotParam in ['EULER_XYZ', 'EULER_ZXZ', 'QUAT_WFIRST', 'EXP_MAP'])
