...
int PymParseCmdlineOptions(pym_cmdline_options_t *cmdopt, int argc, char *argv[]) {
    ...
    if (cmdopt->notrack == 0 && cmdopt->trajconf == 0 && cmdopt->trajdata == 0) {
        ...
        prefixLen = (int)(strchr(cmdopt->simconf, '.') - cmdopt->simconf);

        cmdopt->trajconf = (char *)malloc(prefixLen + strlen(".traj.conf") + 1);
        strncpy(cmdopt->trajconf, cmdopt->simconf, prefixLen);
        cmdopt->trajconf[prefixLen] = '\0';
        strcat(cmdopt->trajconf, ".traj.conf");

        cmdopt->trajdata = (char *)malloc(prefixLen + strlen(".traj_EXP_q.txt") + 1);
        ...
        if (cmdopt->output == 0) {
            ...
            cmdopt->output = (char *)malloc(prefixLen + strlen(".sim_EXP_q.txt") + 1);
