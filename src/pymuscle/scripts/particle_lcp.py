...
def Draw ():
	global rl, vl, n, h, m, D, alpha_0, Ms
	
	q1 = (dot(n, rl) - alpha_0)/h + dot(n, vl + h/m*fg)
	q2 = dot(D.T, vl + h/m*fg)
	q = array([q1] + list(q2) + [0.])
	...
	rl2 = rl + h*(vl+h/m*(cn*n+dot(D, beta)+fg))	
	vl2 = vl + h/m*(cn*n + dot(D, beta) + fg)
...
M = zeros((10,10))
M[0,0] = 1.0/m*dot(n,n)
M[0,1:9] = 1.0/m*dot(n, D)
M[1:9,0] = 1.0/m*dot(D.T,n)
M[1:9,1:9] = 1.0/m*dot(D.T, D)
M[1:9,9] = e
M[9,0] = mu
M[9,1:9] = -e
...
M2 = zeros((10,10))
M2[0:8,0:8] = 1.0/m*dot(D.T, D)
M2[0:8,8] = 1.0/m*dot(D.T,n)
M2[0:8,9] = e
M2[8,0:8] = 1.0/m*dot(n, D)
M2[8,8] = 1.0/m*dot(n,n)
M2[9,0:8] = -e
M2[9,8] = mu
...
#M2u,M2v = linalg.eig(M2)
#print M2u

# We will use 'M' as a weighting matrix for calculating the norm of vector 'x',
# i.e., x^T * M * x, M can be always transformed to symmetric matrix
# 'Ms = 0.5*(M + M^T)' since x^T * M * x equals 0.5 * x^T * (M + M^T) * x.
Ms = 0.5*(M + M.T)

#Msu,Msv = linalg.eig(Ms)
#print Msu

M2s = 0.5*(M2 + M2.T)
print M2s
M2su, M2sv = linalg.eig(M2s)
print M2su
