  ...
  pym_traj_t *pymTraj)
{
  ...
  * to the initial frame of trajectory data. */
  const int prevFrameIdx = -1;
  const int curFrameIdx  = 0;
  const int nextFrameIdx  = 1;
  int i, j;
  double totMass = 0;
  FOR_0(i, pymCfg->nBody) {
    ...
    current = pymTraj->trajData + curFrameIdx*pymTraj->nBlenderBody*6
      + pymTraj->corresMapIndex[i]*6 + 0;
    /* next step */
    next = pymTraj->trajData + nextFrameIdx*pymTraj->nBlenderBody*6
      + pymTraj->corresMapIndex[i]*6 + 0;
    // Current state
    memcpy(rbn->p, current + 0, sizeof(double)*3);
    memcpy(rbn->q, current + 3, sizeof(double)*4);
    /* update discrete velocity based on current and next step */
    FOR_0(j, 3) {
      rbn->pd[j] = ( *(next + 0) - *(current + 0) ) / pymCfg->h;
    }
    FOR_0(j, 4) {
      rbn->qd[j] = ( *(next + 3) - *(current + 3) ) / pymCfg->h;
    }
  }
