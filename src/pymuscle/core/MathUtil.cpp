...
float PymNormf(int dim, const float *v)
{
  float len = 0;
  int i;
  for (i = 0; i < dim; ++i)
...
void Invert6x6MassMatrix(double Minv[6][6], const double M[6][6])
{
  ...
  Minv[1][1] = 1./M[1][1];
  Minv[2][2] = 1./M[2][2];
  determinant =+M[3][3]*(M[4][4]*M[5][5]-M[5][4]*M[4][5])
...
void _TransformPoint(double pt[3], const double W[4][4],
		     const double p[3], int affineAssert) {
  /*
   *    /                   \ /      \     /       \
...
void AffineTransformPoint(double pt[3], const double W[4][4],
			  const double p[3]) {
