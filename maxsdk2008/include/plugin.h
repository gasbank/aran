...
//
/*! \brief This class provides information about a DLL.
	This class is available in release 2.0 and later only.
	This class provides information about a DLL. Every DLL in 3ds Max may implement
	any number of classes. This class has methods which return the number of
	classes implemented in the DLL, a description string for the DLL, and a method
	to unmap the DLL from memory.
	All methods of this class are implemented by the system.
	\sa  Class DllDir, Class ClassDesc, Class Interface, <a href="ms-its:3dsmaxsdk.chm::/start_dll_functions.html">DLL Functions and Class Descriptors</a>.
*/
...
public:
	...
	CoreExport const MCHAR*	Description() { return (*vDescription)(); };
	...
	CoreExport BOOL Initialize() { return !vLibInitialize || (*vLibInitialize)(); }
	//! \brief Function that gets called when the DLL is unloaded.
	/*!	\return The system doesn't pay attention to a return value.*/
	CoreExport BOOL Shutdown() { return !vLibShutdown || (*vLibShutdown)(); }
	};
...
class ClassEntry: public MaxHeapOperators {
	...
	public:
		...
		*/
		...
		/*! \remarks Returns a pointer to the class descriptror for this
		entry. Note: With the addition of deferred-loaded plug-ins introduced
		in 3ds Max 3.0, this method may return a pointer to an instance of
		class <b>DataClassDesc</b> (a sub-class of <b>ClassDesc</b>). A
		developer can tell if the returned class is a <b>DataClassDesc</b>
		object if <b>ClassEntry::IsLoaded()</b> returns FALSE. This derived
		class implements <b>ClassDesc::Create()</b> by returning NULL.
		Developers need to call <b>ClassEntry::FullCD()</b> in order to ensure
		that the class is actually present. Developers can call this method to
		retrieve data about the plug-in without forcing a load. It may also be
		called to get a 'full' <b>ClassDesc</b> if it is known that the plug-in
		is loaded (<b>IsLoaded()</b> returns TRUE). */
...
/*! \brief This class provides a table of SubClassList objects, one for each pluggable super class.
...
class ClassDirectory: public MaxHeapOperators {
	...
	public:				  
...
#endif
		/*! \remarks Returns a pointer to the class descriptor for the class
		...
		CoreExport SubClassList* GetClassList(SClass_ID superClassID);	
#endif
		/*! \remarks Returns a pointer to the class entry for the class whose
		...
		/*! \remarks Returns a reference to the 'i-th' sub class list.
		\par Parameters:
		<b>int i</b>
		Specifies which sub class list to return a reference to. This value is between
		<b>0</b> and <b>Count()-1</b>. */
		SubClassList& operator[](int i){ 
			return(*cl[i]);
		}
		/*! \remarks This method is used internally <b>only</b> to add a new
		Super Class ID to the list maintained by this class.
		<b>Important Note</b>: It is illegal to create your own Super Class of
		persistent objects in 3ds Max. 3ds Max will crash if a scene is saved
		and later loaded into a system that doesn't have a plug-in added Super
		Class. The problem is that 3ds Max needs a stand-in class for each
		...
		listed <a href="ms-its:listsandfunctions.chm::/idx_R_list_of_super_class_ids.html">List of Super Class
		IDs</a>. */
		...
		/*! \remarks This method is used internally to add a class descriptor to the
		list maintained by this class.
		\par Operators:
		*/
		CoreExport int  AddClass(ClassDesc *cdesc, int dllNum, int index, bool load);
		/*! \remarks Returns the number of class lists in the class directory.		*/
...
/*! \brief A partial class desciptor for classes in plug-ins but described by Registry entries
	...
	other functionality. Note: With the addition of delay-loaded plugins, the method ClassEntry::CD() may
