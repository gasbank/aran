...
class INode: public ReferenceTarget, public FPMixinInterface {
	public:
		...
		*/
		...
		\param keepPos If nonzero, the position of the child node is retained (not moved); 
		otherwise it may be moved. */
		...
		\param keepPos If nonzero, the position of the detached node is retained (not moved); 
		otherwise it may be moved. */
...
// mjm - end
		...
		\param type See <a href="ms-its:listsandfunctions.chm::/idx_R_transform_lock_types.html">Transform Lock Types</a>.
		\param axis See <a href="ms-its:listsandfunctions.chm::/idx_R_transform_lock_axis.html">Transform Lock Axis</a>.
...
//		virtual void GetMaterial(Material &mtl)=0;  // Why do we need this?
		...
		\return See <a href="ms-its:listsandfunctions.chm::/idx_R_list_of_xref_flag_bits.html">List of XRef Flag Bits</a>.
		\par Default Implementation:
		<b>{return 0;}</b> */
		...
		\param flag See <a href="ms-its:listsandfunctions.chm::/idx_R_list_of_xref_flag_bits.html">List of XRef Flag Bits</a>.
		\param onOff TRUE for on; FALSE for off.
		\par Default Implementation:
		<b>{}</b>        */
...
#define XREF_SCENE_OVERLAY	(1<<12)  // watje this bit is set when a scene xref is set to overlay.  An overlay xref will only be loaded if it a direct descendant of the master file.  This lets the user bring in files to compare against but other users wont see if they xref this file.
